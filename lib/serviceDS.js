...
class serviceDS {
    ...
    async readOldProjectData (params) {
        try {
            var me = this;
            var query = {'_id': ObjectId(params)}
            var result = await mongoDs.collection("projectInfo_1_0").find(query).toArray();
            console.log('monog function is triggered');
    ...
    async readAllProjectInfo () {
        try {
            var me = this;
            console.log('monog function is triggered');
            var result = await mongoDs.collection("projectInfo_1_0").find({}).toArray();
            console.log('monog function is triggered');
            console.log(result);
            return result;
        } catch (error) {
    ...
    async storeProjectInfo (data) {
        try {
            var me = this;
            console.log('monog function is triggered');
            var result = await mongoDs.collection("projectInfo_1_0").insertOne(data);
            //console.log(result);
            return result;
        } catch (error) {
            throw error;
