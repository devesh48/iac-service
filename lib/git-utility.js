...
class gitUtility {
  ...
  async init(data) {
    if (data['gitRepoName'] && data['gitRepoName'] !== '') {
      console.log(data['gitRepoName']);
    } else {
      this.git = simpleGit('gitSource', { binary: 'git' });
      this.git.addConfig('user.email', 'upadhyay.yuvraj48@gmail.com', false, 'global')
      this.git.addConfig('user.name', 'acc-github-repo', false, 'global')
    }
  }
